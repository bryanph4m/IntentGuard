...
describe("live run adapters", () => {
  it("starts development runs without POST and subscribes to every named SSE event", async () => {
  ...
  it("keeps real API run creation and approval POST-based", async () => {
    const fetchMock = vi.fn<typeof fetch>()
    ...
    const created = await adapter.createRun();
    await adapter.approve(created.runId, { reviewer: "Bryan", comment: "Ship it." });
